 ...
 * @since 2.0.0
 */
class User_Profile_Picture_Gutenberg {
	/**
	 * Class constructor.
	 */
			...
			array(
				'attributes'      => array(
					...
					'profileViewPosts'                => array(
						'type'    => 'string',
						'default' => __( 'View Posts', 'profile-builder' ),
					),
					'profileViewWebsite'              => array(
						'type'    => 'string',
						'default' => __( 'View Website', 'profile-builder' ),
					),
					...
					'tabbedAuthorProfile'             => array(
						'type'    => 'string',
						'default' => __( 'Author Details', 'profile-builder' ),
					),
					...
					'tabbedAuthorLatestPosts'         => array(
						'type'    => 'string',
						'default' => __( 'Latest Posts', 'profile-builder' ),
					),
								...
								<div class="mpp-gutenberg-view-posts">
									<?php if ( $attributes['showViewPosts'] ) : ?>
										<div class="mpp-profile-view-posts" style="background-color: <?php echo esc_attr( $attributes['profileViewPostsBackgroundColor'] ); ?>; color: <?php echo esc_attr( $attributes['profileViewPostsTextColor'] ); ?>; width: <?php echo esc_attr( $attributes['showPostsWidth'] ); ?>; font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
											...
											if ( isset( $attributes['profileViewPosts'] ) ) {
												...
												esc_html_e( 'View Posts', 'profile-builder' );
											}
											?>
											</a>
									...
									<div class="mpp-profile-view-website" style="background: <?php echo esc_attr( $attributes['profileWebsiteBackgroundColor'] ); ?>;color: <?php echo esc_attr( $attributes['profileWebsiteTextColor'] ); ?>; font-size: <?php echo esc_attr( $attributes['buttonFontSize'] ); ?>px;">
										...
										if ( isset( $attributes['profileViewWebsite'] ) ) {
											...
											esc_html_e( 'View Website', 'profile-builder' );
										}
										?>
										</a>
	...
	public function add_gutenberg_scripts() {
		...
		wp_enqueue_script( 'mpp_gutenberg', User_Profile_Picture::get_plugin_url( 'dist/blocks.build.js' ), array( 'wp-blocks', 'wp-element' ), USER_PROFILE_PICTURE_VERSION, true );

		/* For the Gutenberg plugin */
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'mpp_gutenberg', 'profile-builder' );
		} elseif ( function_exists( 'gutenberg_get_jed_locale_data' ) ) {
			$locale  = gutenberg_get_jed_locale_data( 'profile-builder' );
			...
			/* for 5.0 */
			$locale  = wp_get_jed_locale_data( 'profile-builder' );
		...
		wp_localize_script(
			...
			array(
				...
				'mystery_man' => User_Profile_Picture::get_plugin_url( 'img/mystery.png' ),
			)
		);

		wp_enqueue_style( 'mpp_gutenberg', User_Profile_Picture::get_plugin_url( '/dist/blocks.editor.build.css' ), array(), USER_PROFILE_PICTURE_VERSION, 'all' );
	}
	...
	public function add_gutenberg_styles() {
		...
		wp_enqueue_style( 'mpp_gutenberg', User_Profile_Picture::get_plugin_url( '/dist/blocks.style.build.css' ), array(), USER_PROFILE_PICTURE_VERSION, 'all' );
		wp_enqueue_script( 'mpp_gutenberg_tabs', User_Profile_Picture::get_plugin_url( 'js/mpp-frontend.js' ), array( 'jquery' ), USER_PROFILE_PICTURE_VERSION, true );
