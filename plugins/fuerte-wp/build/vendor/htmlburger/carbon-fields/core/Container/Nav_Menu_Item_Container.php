...
class Nav_Menu_Item_Container extends Container
{
    ...
    {
        include \FuerteWpDep\Carbon_Fields\DIR . '/templates/Container/nav_menu_item.php';
    }
    /**
    ...
    protected function get_clone_for_menu_item($menu_item_id, $load = \true)
    {
        if (!isset($this->menu_item_instances[$menu_item_id])) {
            ...
            $container = Container::factory($this->type, $menu_item_field_prefix . $this->get_id())->set_datastore($menu_item_datastore, \true)->add_fields($custom_fields)->init($menu_item_id);
            if ($load) {
                $container->load();
            }
    ...
    {
        return 'FuerteWpDep\\Carbon_Fields\\Walker\\Nav_Menu_Item_Edit_Walker';
    }
}
