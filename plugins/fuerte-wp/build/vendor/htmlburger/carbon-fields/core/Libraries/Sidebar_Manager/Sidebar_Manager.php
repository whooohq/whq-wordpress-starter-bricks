...
namespace FuerteWpDep\Carbon_Fields\Libraries\Sidebar_Manager;
...
class Sidebar_Manager
{
    ...
    public function boot()
    {
        ...
        if (\function_exists('FuerteWpDep\\crb_get_default_sidebar_options')) {
    ...
    public function execute_action($action, $input)
    {
        ...
        if (empty($name)) {
            return new \FuerteWpDep\WP_Error('name-missing', __('Please enter a name for the sidebar.', 'carbon-fields'));
        }
        $result = new \FuerteWpDep\WP_Error('unknown-action', __('Unknown action attempted.', 'carbon-fields'));
        switch ($action) {
    ...
    public function add_sidebar($name, $id = '')
    {
        ...
        if (isset($registered_sidebars[$id])) {
            return new \FuerteWpDep\WP_Error('sidebar-exists', __('Sidebar with the same ID is already registered.', 'carbon-fields'));
        }
        $registered_sidebars[$id] = array('id' => $id, 'name' => $name);
        $success = update_option('carbon_custom_sidebars', $registered_sidebars);
        if (!$success) {
            return new \FuerteWpDep\WP_Error('update-failed', __('Failed to update option storing your custom sidebars. Please contact support.', 'carbon-fields'));
        }
    ...
    public function remove_sidebar($id)
    {
        ...
        if (isset($registered_sidebars[$id])) {
            ...
            return new \FuerteWpDep\WP_Error('sidebar-not-found', __('Sidebar not found.', 'carbon-fields'));
        }
        $success = update_option('carbon_custom_sidebars', $registered_sidebars);
        if (!$success) {
            return new \FuerteWpDep\WP_Error('update-failed', __('Failed to update option storing your custom sidebars. Please contact support.', 'carbon-fields'));
    ...
    {
        wp_enqueue_style('carbon-sidebar-manager', \FuerteWpDep\Carbon_Fields\URL . '/core/Libraries/Sidebar_Manager/assets/css/app.css', array(), \FuerteWpDep\Carbon_Fields\VERSION);
        wp_enqueue_script('carbon-sidebar-manager', \FuerteWpDep\Carbon_Fields\URL . '/core/Libraries/Sidebar_Manager/assets/js/app.js', array(), \FuerteWpDep\Carbon_Fields\VERSION);
