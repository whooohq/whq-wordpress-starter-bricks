...
class Map_Field extends Field
{
    ...
    public function to_json($load)
    {
        ...
        $field_data = \array_merge($field_data, array('value' => array('lat' => \floatval($value_set['lat']), 'lng' => \floatval($value_set['lng']), 'zoom' => \intval($value_set['zoom']), 'address' => $value_set['address'], 'value' => $value_set[Value_Set::VALUE_PROPERTY])));
        return $field_data;
    }
    /**
    ...
    {
        return $this->set_default_value(\array_merge($this->get_default_value(), array(Value_Set::VALUE_PROPERTY => $this->lat_lng_to_latlng($lat, $lng), 'lat' => $lat, 'lng' => $lng, 'zoom' => $zoom)));
    }
}
