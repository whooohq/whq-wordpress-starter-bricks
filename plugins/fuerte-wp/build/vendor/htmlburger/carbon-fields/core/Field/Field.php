...
class Field implements Datastore_Holder_Interface
{
    ...
    public static function factory($raw_type, $name, $label = null)
    {
        ...
        if (Carbon_Fields::has($type, 'fields')) {
            return Carbon_Fields::resolve_with_arguments($type, array('type' => $type, 'name' => $name, 'label' => $label), 'fields');
        }
    ...
    public function is_simple_root_field()
    {
        ...
        return empty($hierarchy) && \in_array($this->get_value_set()->get_type(), array(Value_Set::TYPE_SINGLE_VALUE, Value_Set::TYPE_MULTIPLE_PROPERTIES));
    }
    /**
    ...
    protected function parse_conditional_rules($rules)
    {
        ...
        $parsed_rules = array('relation' => Helper::get_relation_type_from_array($rules), 'rules' => array());
        $rules_only = $rules;
