...
use FuerteWpDep\Carbon_Fields\Loader\Loader;
use FuerteWpDep\Carbon_Fields\Container\Repository as ContainerRepository;
use FuerteWpDep\Carbon_Fields\Toolset\Key_Toolset;
use FuerteWpDep\Carbon_Fields\Toolset\WP_Toolset;
...
use FuerteWpDep\Carbon_Fields\Service\Revisions_Service;
...
use FuerteWpDep\Carbon_Fields\REST_API\Router as REST_API_Router;
use FuerteWpDep\Carbon_Fields\REST_API\Decorator as REST_API_Decorator;
use FuerteWpDep\Carbon_Fields\Event\Emitter;
use FuerteWpDep\Carbon_Fields\Event\PersistentListener;
use FuerteWpDep\Carbon_Fields\Event\SingleEventListener;
...
final class Carbon_Fields
{
    ...
    {
        $url = \FuerteWpDep\trailingslashit($directory);
        $count = 0;
        # Sanitize directory separator on Windows
        $url = \str_replace('\\', '/', $url);
        $possible_locations = array(
            WP_PLUGIN_DIR => \FuerteWpDep\plugins_url(),
            # If installed as a plugin
            WP_CONTENT_DIR => \FuerteWpDep\content_url(),
            # If anywhere in wp-content
            ABSPATH => \FuerteWpDep\site_url('/'),
        );
        foreach ($possible_locations as $test_dir => $test_url) {
            ...
            if ($count > 0) {
                return \FuerteWpDep\untrailingslashit($url);
            }
        }
        return '';
    ...
    protected static function get_default_ioc()
    {
        ...
        $ioc->register(new \FuerteWpDep\Carbon_Fields\Provider\Container_Condition_Provider());
        return $ioc;
    }
}
