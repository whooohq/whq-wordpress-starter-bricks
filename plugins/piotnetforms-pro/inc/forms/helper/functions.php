...
class Piotnetforms_Helper {
	...
	public function mailchimp_curl_get_connect( $url, $request_type, $api_key, $data = [] ) {
		if ( $request_type == 'GET' ) {
	...
	public function piotnetforms_constant_contact_create_contact( $token, $data ) {
		$curl = curl_init();
		curl_setopt_array( $curl, [
	...
	public function piotnetforms_constant_contact_refresh_token( $key, $secret, $refresh_token ) {
		$curl = curl_init();
		curl_setopt_array( $curl, [
		...
		if ( !empty( $response ) && !empty( $response->access_token ) ) {
			update_option( 'piotnetforms-constant-contact-access-token', $response->access_token );
			update_option( 'piotnetforms-constant-contact-refresh-token', $response->refresh_token );
	...
	public function piotnetforms_paypal_get_token( $client_id, $client_secret, $paypal_url ) {
		$curl = curl_init();
		curl_setopt_array( $curl, [
