...
namespace Gravity_Forms\Gravity_Forms_Conversational_Forms\Util\Colors;
...
class Color_Modifier {
	...
	public function rgb_to_hsl( $r, $g, $b ) {
		...
		return array( 'h' => round( $h ), 's' => round( $s ), 'l' => round( $l ) );
	}
	...
	public function hsv_to_rgb( $h, $s, $v ) {
		...
		return array( 'r' => round( 255 * ( $r + $m ) ), 'g' => round( 255 * ( $g + $m ) ), 'b' => round( 255 * ( $b + $m ) ) );
	}
